...
exportState, operatorChat, saveOperatingRules, resyncConnection,
resetDemoWorkspace. The rate limiter (`lib/server/rate-limit.ts`) is an
...
## Server/client boundary (P2/P3)

- Client components import server actions **only** from "use server" modules
  (e.g. `@/lib/server/actions`), never the broad `lib/server` barrel.
...
version. All of this is pinned by `tests/export-privacy.test.ts`
